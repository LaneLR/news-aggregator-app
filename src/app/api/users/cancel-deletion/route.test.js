...
function makeRequest(body) {
  return new NextRequest("http://localhost/api/users/cancel-deletion", {
    method: "PATCH",
    body: JSON.stringify(body),
  });
}

describe("PATCH /api/users/cancel-deletion", () => {
  beforeEach(() => {
    mockAuth.mockReset();
  ...
  it("cancels the pending deletion and sends a confirmation email", async () => {
    mockAuth.mockResolvedValue(makeSession({ id: "user-1" }));
    ...
    expect(user.update).toHaveBeenCalledWith({ isPendingDeletion: false, deletionRequestedAt: null });
    expect(mockSendEmail).toHaveBeenCalledWith(
      expect.objectContaining({ to: "a@example.com", subject: expect.stringContaining("Canceled") })
    );
  });
