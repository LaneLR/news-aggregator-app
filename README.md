...
- **The "no image" state is a rendered gradient card, not a fallback image file.** `ArticleFallbackArt.jsx` replaces an article's missing (or broken) thumbnail with a `linear-gradient` in that category's own color (`getCategoryColor()` in `src/lib/categoryColors.js`, via CSS `color-mix()` for the second stop — no per-category asset to keep in sync), with the article's own headline overlaid where the layout has room for it (`NewsCardThree`/`NewsCardFour`) or left plain where the title's already shown alongside it (the shared-archive page's small thumbnails, `CarouselArticleCard`'s own existing title overlay). Replaces an earlier version of this that used one hand-tinted PNG per category (`public/images/placeholders/*.png`) — that read as a broken/missing image to users regardless of how deliberate the color choice was, since an icon-on-a-color-block is exactly what a broken `<img>` looks like.
...
| `MAINTENANCE_MODE` | Incident response | Optional, unset by default. Set to the literal string `true` (in Vercel's dashboard, then redeploy) to take the whole site down to a static maintenance page — see `src/proxy.js`. Unset it and redeploy again to bring the site back. |
